...
#==================== Start of Short Only Strategy ====================
# TODO: NEED TO UPDATE THIS TO REFLECT THE CHANGES MADE TO THE STRATEGY ABOVE. 
...
class SHORTING_Underwater_w_decay_and_deleverage(Strategy):
    ...
    def next(self):
        ...
        if self.position.size < 0 and self.position.pl_pct < -atr_threshold_pct:
            # Check to see if we are also down on our last trade
            if self.trades[-1].pl_pct < -atr_threshold_pct:
                self.sell(size=self.add_size)
...
# ================= END OF SHORT ONLY STRATEGY CODE ====================



# ================= START OF LONG ONLY STRATEGY CODE ====================

# TODO: Need to refresh this and bring it in line with the main long-short strategy. I've added more params etc. to that one so this is a bit older.

class LONG_ONLY_Underwater_w_decay_and_deleverage(Strategy):
    # All of the following variables can be used during optimization
    initial_position_size = 0.3
    percent_invested_threshold = 0.50
    atr_length = 14  # 14 days
